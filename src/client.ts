// Read-only gateway client. Two GETs, nothing is ever sent up except the API
// key header. Never throws: every failure comes back as a typed result so the
// run loop can hold instead of crashing.
import { parseConfig, parseTarget, STRATEGY_ID, type ConfigDoc, type ParseResult, type TargetDoc } from "./protocol/index.js";
...
export type FetchResult<T> = { ok: true; value: T } | { ok: false; kind: FetchFailureKind; message: string };
...
async function getJson<T>(opts: GatewayOptions, path: string, parse: (input: unknown) => ParseResult<T>): Promise<FetchResult<T>> {
  ...
  if (response.status !== 200) {
    ...
    if (response.status === 401) return { ok: false, kind: "auth", message: "The API key is unknown, revoked or expired." };
    if (response.status === 403) return { ok: false, kind: "auth", message: "The API key is not allowed to use this strategy." };
