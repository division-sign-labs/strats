...
| Your own theme (`theme`) | Polymarket | Shares in the markets that fit your theme, chosen by Q (or, on a legacy key, the ones you chose), each bought once and sold at a take-profit price or when the market closes |
...
The server names an outcome only when Q has forecast the market, the outcome's price is at least 0.70, and Q's probability is at least 5 points above it; the limit is Q less 5 points, at most 0.97, and shares are held to resolution. The runner checks each target again before it trades: Q's forecast is present, the limit is between 0.70 and 0.97, and Q is at least 5 points above it. When the order is placed the same floor and the same 5 points are held against the live ask, so an outcome that has fallen under 0.70 since the targets were computed is not bought. On a legacy key the market must be one you chose and the outcome one your direction allows (long: the outcome good for a long; short: the other; both: either). On a managed key (`universe: "managed"`, the default for new keys) Q chooses the markets, adds new ones and drops ended ones; the runner trades any market the targets name once Polymarket itself confirms the token belongs to that market, and which outcome suits your direction is the server's word. A theme key works the same way. A target that fails is refused in the log, and its market is still managed so a holding can be sold or redeemed.
...
The checks cover the addresses, chains, amounts and the pinned contract in LI.FI's answer, and the exact allowance limits a bad route to one payout. Where the transaction's calldata decodes (LI.FI's same-chain swap and its bridge call), the recipient, the destination chain and, for a same-chain swap, the minimum are also read from the transaction you sign, and a mismatch is refused. For a bridge route the token minimum is often only the route's promise; when it is not written into the transaction, or the transaction cannot be decoded, the plan says so in a "Not checked" row. The token bought on the far side of a bridge is not checked in the calldata.
...
Auto-buyback is for single-asset bots only. A theme or team bot measures profit from a deposits figure kept on your machine, and Polymarket shows no deposit history to check it against, so a droplet could pay out money you added as if it were profit. `init` and `config` do not offer it for those bots, a droplet refuses it, and `strats deploy` turns it off on a bot that had it on from 0.5.0. `strats buyback --execute`, with its y/N, works as before.

What you give up: a single-asset bot's droplet normally holds a trading key that cannot withdraw. With auto-buyback on it holds the wallet's master key. It is still your droplet and your keys; Quotient holds nothing.

What the droplet does: once every 24 hours (first 10 minutes after its first start; a restart waits out the rest of the day) it runs the same buyback as `strats buyback --execute`, with the same arithmetic, the $25 minimum, 1% slippage, the 3% price-impact limit, the same quote and gas checks, and the same record. Unattended it is stricter in three ways: it pays nothing when the profit to split is more than 25% of the account value in one check, it uses only routes whose signed transaction carries the recipient and the token minimum, and it refuses a transaction it cannot decode. A buyback that is part-way is continued before anything new is planned. No question is asked, because nobody is there: the y you gave to `init` or `config` is the consent, and the code refuses unless the bot file says `autoBuyback` and it is running from the credentials `strats deploy` sent. It runs beside the trading loops, never inside a cycle, one at a time. An error is one log line, and the next daily check is the retry. Each line starts with `buyback`, and a finished one reads "Bought 1,000,000 TKN for $349.00. ...". The next report's `boughtBackUsd` includes it.

One place pays. While auto-buyback is on and the bot is deployed, `strats buyback --execute` on your machine refuses: "The droplet does the buybacks for this bot ...". The dry run still works and first copies the droplet's payout lines, so "Already split" is right. `strats status` shows the droplet's last buyback line. To do one by hand, turn it off and deploy; that deploy stops the runner, brings the droplet's payout record home, and moves a part-way buyback to your machine, where `strats buyback --execute` finishes it. `strats destroy` brings the record home too. If the droplet cannot be read, both refuse rather than lose the only copy; `--force` goes on without it and turns auto-buyback off, so the new droplet does not buy back from a record that may be missing payouts; check "Already split" before you say y to a buyback, and before you turn it on again. `strats fund` refuses while a buyback is part-way, here or on the droplet, because it would deposit that buyback's money back into Hyperliquid. A deploy that would replace the droplet while a buyback is part-way is refused.
